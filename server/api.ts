// api.ts - The forward facing APIs to access Toby's data
...
import { IVideoGroup, IVideoEntry } from "./common";
...
export default class API {
  ...
  constructor(db: DB, server: express.Express) {
    this.db = db;
    this.server = server;
    ...
    this.routes = [
      ...
      },
      {
        ...
        route: this.postVideosRecentlyPlayedLast30
      }
    ];

    this.db.importIntoDB(DefaultData.getData());

    this.initializeRoutes(this.server, this.routes);
  }
  private initializeRoutes(server: express, routes: APIRoute[]): void {
    _.forEach(routes, r => {
      let routePath = r.path.split(" ");
      server[routePath[0].toLowerCase()](routePath[1], r.route.bind(this));
    });
  }
  ...
  private postVideosRecentlyPlayedAdd(req: express.Request, res: express.Response): void {
    ...
    if (title !== undefined && title.length > 0 && ytid !== undefined && ytid.length > 0) {
  ...
  private postVideosRecentlyPlayedLast30(req: express.Request, res: express.Response): void {
    let trim = false;
