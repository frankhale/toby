...
import { IVideoGroup } from "./common";
...
export default class DB {
  ...
  importIntoDB(videoData: IVideoGroup[]): void {
    this.db.serialize(() => {
      this.db.run(
        "CREATE TABLE IF NOT EXISTS videos (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, ytid  TEXT, [group] TEXT)"
      );

      this.db.each("SELECT COUNT(*) as count FROM videos", (_err, _row) => {
          ...
          _.forEach(g.entries, e => {
            this.getVideoFromDB(e.ytid, row => {
              if (row === undefined && g.group !== "Recently Played") {
                ...
                this.db.run("INSERT into videos(title,ytid,[group]) VALUES (?,?,?)", [
                  e.title,
                  e.ytid,
                  g.group
                ]);
  ...
  getAllVideosOrderedByGroupDB(finished: (rows: any[]) => void): void {
    this.db.all("SELECT title, ytid, [group] FROM videos ORDER BY [group]", (_err, rows) => {
      if (finished !== undefined) {
        finished(rows);
      }
    });
  ...
  getAllGroupsFromDB(finished: (rows: any[]) => void): void {
    this.db.all("SELECT DISTINCT [group] FROM videos", (_err, rows) => {
      if (finished !== undefined) {
        finished(rows);
      }
    });
  ...
  getVideosFromGroupWhereTitleLikeFromDB(
    searchTerm: string,
    group: string,
    finished: (rows: any[]) => void
  ): void {
    searchTerm = `%${searchTerm.trim()}%`;
  ...
  deleteVideoFromDB(ytid: string): void {
    this.db.get("SELECT ytid FROM videos WHERE ytid = ?", [ytid], (_err, rows) => {
      if (rows !== undefined) {
  ...
  updateVideoFromDB(title: string, ytid: string, group: string): void {
    if (title !== undefined || (title !== "" && group !== undefined) || group !== "") {
      this.db.get("SELECT ytid FROM videos WHERE ytid = ?", [ytid], (_err, rows) => {
        if (rows !== undefined) {
          this.db.run("UPDATE videos SET title = ?, group = ? WHERE ytid = ?", [
            title,
            group,
            ytid
          ]);
