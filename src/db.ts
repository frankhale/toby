...
export default class DB {
  ...
  getAllVideosWhereYTIDInList(
    ytids: string[],
    finished: (rows: any[]) => void
  ): void {
    let ytids_in_string = _.map(ytids, r => {
      return `'${r}'`;
    }).join(",");
            ...
            (err, ytids_found) => {
              let _ytids = _.map(ytids_found, r => {
                ...
                _rows = _.forEach(rows, d => {
                  d.isArchived =
                    _.indexOf(_ytids, d.ytid) !== -1 ? true : false;
                });
